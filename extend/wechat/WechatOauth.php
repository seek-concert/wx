...
class WechatOauth extends Common {
    ...
    public function getUserInfo()
    {
        ...
        if (!isset($_GET['code'])){
           ...
           $data['user_name']=empty($data2['nickname']) ? '微信用户' : trim($data2['nickname']);
            $data['sex'] = $data2['sex'];
            $data['avatar'] = $data2['headimgurl'];
     ...
     * @return string
     */
    protected function getOauthRedirect($callback, $state = '', $scope = 'snsapi_base') {
     ...
     */
    protected function getOauthUserInfo($access_token, $openid) {
